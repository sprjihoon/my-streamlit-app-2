...
from .clean import TRACK_COLS, clean_invoice_id
...
def add_basic_shipping(
    ...
    Args:
        df_items: 기존 인보이스 항목 DataFrame
        vendor: 공급처명
        d_from: 시작일 (YYYY-MM-DD)
        d_to: 종료일 (YYYY-MM-DD)
    
    Returns:
        기본 출고비 항목이 추가된 DataFrame
    """
    with get_connection() as con:
        ...
        date_col = next(
            (c for c in ["배송일", "송장등록일", "출고일자", "기록일자", "등록일자"]
        ...
        df_raw = df_raw[
            (df_raw[date_col] >= pd.to_datetime(d_from)) &
            (df_raw[date_col] <= pd.to_datetime(d_to))
        ]

        # 중복 출고 행 제거 (송장번호가 있을 때만)
...
def add_courier_fee_by_zone(
    ...
    Returns:
        구간별 수량 딕셔너리 {구간명: 수량}
    """
    from .fee_courier import calculate_courier_fee_by_zone as _impl
...
# 3. 인보이스 DB 유틸
# ─────────────────────────────────────────────
def get_invoice_id(vendor_id: int, d_from: str, d_to: str) -> Optional[int]:
    """기존 인보이스 ID 조회."""
    with get_connection() as con:
        row = con.execute(
            "SELECT invoice_id FROM invoices "
            "WHERE vendor_id=? AND period_from=? AND period_to=?",
            (vendor_id, d_from, d_to)
        ).fetchone()
...
def create_and_finalize_invoice(
    vendor_id: int,
    period_from: str,
    period_to: str,
    items: List[Dict],
    ...
    with get_connection() as con:
        ...
        for it in items:
            try:
                ...
                if amount > 9000000000000000000:
                    warnings.append(f"⚠️ 금액 초과 항목 제외: {it['항목']} ({amount})")
                    amount = 0
                ...
                warnings.append(f"⚠️ 데이터 변환 오류 항목 제외: {it} - {e}")
        ...
        cur.executemany(
            ...
            "VALUES ( ?, ?, ?, ?, ?, ? )",
            [
                (iid, it["항목"], it["수량"], it["단가"], it["금액"], it["비고"])
...
def get_extra_unit(label: str) -> int:
    ...
    with get_connection() as con:
        try:
            val = pd.read_sql(
                "SELECT 단가 FROM out_extra WHERE 항목=?",
                con, params=(label,)
            ).squeeze()
            if pd.notna(val):
...
def get_material_unit(label: str) -> int:
    ...
    with get_connection() as con:
        try:
            val = pd.read_sql(
                "SELECT 단가 FROM material_rates WHERE 항목=?",
                con, params=(label,)
            ).squeeze()
            if pd.notna(val):
                return int(float(val))
...
def add_barcode_fee(items: List[dict], vendor: str) -> None:
    """바코드 부착 요금 추가."""
    add_flag_fee(items, vendor, "barcode_f", "바코드 부착", "입고검수", get_extra_unit)


def add_void_fee(items: List[dict], vendor: str) -> None:
    """완충작업 요금 추가."""
    add_flag_fee(items, vendor, "void_f", "완충작업", "기본 출고비", get_extra_unit)


def add_ppbag_fee(items: List[dict], vendor: str) -> None:
    """PP 봉투 요금 추가."""
    add_flag_fee(
        items, vendor, "pp_bag_f", "PP 봉투", "입고검수",
        lambda _: get_material_unit("PP 봉투 중형")
    )


def add_video_out_fee(items: List[dict], vendor: str) -> None:
    """출고영상촬영 요금 추가."""
    add_flag_fee(items, vendor, "video_out_f", "출고영상촬영", "기본 출고비", get_extra_unit)


# ─────────────────────────────────────────────
# 6. 반품 관련 액션
# ─────────────────────────────────────────────
def add_return_pickup_fee(
    items: List[dict],
    vendor: str,
    d_from: str,
    d_to: str
    ...
    with get_connection() as con:
        ...
        cnt = _count_kpost_ret(con, names, d_from, d_to)
    if cnt:
...
def add_return_courier_fee(
    ...
    with get_connection() as con:
        ...
        if df.empty:
            return
        df["우편물부피"] = pd.to_numeric(df["우편물부피"], errors="coerce").fillna(0)
        zone = (
            pd.read_sql("SELECT * FROM shipping_zone WHERE 요금제=?", con, params=(rate,))
            .sort_values("len_min_cm")
        )

    for _, z in zone.iterrows():
        cnt = df[
            (df["우편물부피"] >= z["len_min_cm"]) &
            (df["우편물부피"] <= z["len_max_cm"])
        ].shape[0]
        if cnt:
            items.append({
                "항목": f"반품 택배요금 ({z['구간']})",
...
def add_box_fee_by_zone(
    ...
    with get_connection() as con:
        ...
        # 2) 단가 테이블
        rates = (
            pd.read_sql("SELECT size_code, 항목, 단가 FROM material_rates", con)
            .set_index("size_code")
        )
        ...
        if want_mailer:
            if size == "극소":
                if "극소" in rates.index:
                    ...
                    df_m = df_search[
                        df_search["항목"].str.contains("택배 봉투", na=False) &
...
def add_worklog_items(
    ...
    # 분류 + 단가 + 비고 동일 행 합계
    df_final = (
        df.groupby([WL_COL_CAT, WL_COL_UNIT, WL_COL_MEMO], as_index=False, sort=False)
...
def add_storage_fee(
    ...
    try:
        ...
        import sys
        print(f"⚠️ 보관료 조회 중 오류 발생 ({vendor}): {e}", file=sys.stderr)
