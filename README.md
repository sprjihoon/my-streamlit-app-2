...
우체국 배송비(`totalFee`)는 마진 없이 그대로 보여 준다. 버퍼와 환율 2%는 이 금액에 붙지 않는다.

미국·영국만 관세 선납(DDP) 예상액을 따로 계산한다. 그 밖의 국가는 배송비만 보여주고, 관세는 수취인이 현지에서 낸다.

- 버퍼 10%는 배송비가 아니라 관세·운송사 수수료·부가세 추정액에만 붙는다. 미국 EMS는 신고가액의 17%와 운송사 수수료, 미국 프리미엄은 신고가액의 20%와 운송사 수수료, 영국은 신고가액의 20% 부가세가 그 기준이다.
- 환율 2%는 그 미국·영국 예상액을 달러에서 원으로 바꿀 때만 붙는다. 다른 국가 배송비에는 없다.
- 최저 금액(15,000 / 25,000 / 10,000원)은 쓰지 않는다. 환산한 뒤 1,000원 단위로만 올린다.
- 배송요금 칸과 접수 확인창에는 DDP 합계와, 그 안에 포함된 버퍼 원화를 같이 보여 준다.
- 미국 신고가액 USD 800 초과는 EMS 프리미엄(FedEx DDP)만 선납 가능하다. 환율은 `EMS_USD_KRW_RATE`, 없으면 1400이다.
...
- **fix(overseas-shipping): DDP 버퍼 10% 통일, 최저 금액 제거**
  - 우체국 배송비는 그대로 두고, 버퍼 10%는 미국·영국 관세·수수료·부가세 추정액에만 붙인다
  - 환율 2%는 그 달러 예상액을 원으로 바꿀 때만 적용한다. 다른 국가 배송비에는 없다
  - 15,000 / 25,000 / 10,000원 바닥값은 쓰지 않고, 1,000원 단위로만 올린다
