...
# 프리패킹
# ─────────────────────────────────────
class PrepackingAnalyzeRequest(BaseModel):
    """프리패킹 조합 분석 요청."""
    vendor: str = Field(..., description="공급처명")
    date_from: date = Field(..., description="시작일")
    date_to: date = Field(..., description="종료일")
...
class PrepackingPredictRequest(BaseModel):
    """프리패킹 예측 요청."""
    vendor: str = Field(..., description="공급처명")
    target_date: date = Field(..., description="예측 대상일")
    weeks_back: int = Field(default=8, description="참조할 과거 주 수")
    save: bool = Field(default=True, description="예측 결과 DB 저장 여부")


class PrepackingProductionCreate(BaseModel):
    """프리패킹 제작 기록 생성."""
    vendor: str
    target_date: date
    combo_key: str
...
class PrepackingProductionUse(BaseModel):
    """프리패킹 사용 (차감)."""
    use_qty: int = Field(..., ge=1, description="사용 수량")
    changed_by: str = Field(default="", description="처리자")


class PrepackingStatusUpdate(BaseModel):
    """프리패킹 상태 변경."""
    status: str = Field(..., description="새 상태 (active/carried/held/disassemble/disassembled/depleted)")
    changed_by: str = Field(default="", description="처리자")


class PrepackingLocationUpdate(BaseModel):
    """로케이션 변경."""
    location: str = Field(..., description="새 위치")
    changed_by: str = Field(default="", description="처리자")


class PrepackingSettingsUpdate(BaseModel):
    """프리패킹 설정 저장."""
    vendor: str = Field(default="_default", description="공급처 (_default=글로벌)")
    min_predicted_qty: int = Field(default=3, ge=1)
    min_frequency: int = Field(default=5, ge=1)
    min_sku_count: int = Field(default=2, ge=1)
    retention_days: int = Field(default=2, ge=1)


class PrepackingAccuracyRequest(BaseModel):
    """정확도 업데이트 요청."""
    vendor: str
    target_date: date
