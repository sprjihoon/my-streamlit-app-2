...
8. "chat_mode_start" - 대화모드 시작 ("대화모드", "대화모드 시작", "챗모드" 등)
9. "chat_mode_end" - 대화모드 종료 ("대화모드 종료", "대화모드 끝", "대화 끝" 등)
10. "work_mode_start" - 작업모드 시작 ("작업모드", "작업모드 시작" 등) - "종료" 없이 "작업모드"만
11. "work_mode_end" - 작업모드 종료 ("작업모드 종료", "작업 끝", "작업모드 끝" 등) - "종료/끝" 포함 시
12. "confirm_yes" - 긍정 응답 (네, 응, 맞아, 그래, ㅇㅇ, 확인 등)
13. "confirm_no" - 부정 응답 (아니, 아니오, 취소, ㄴㄴ, 안해 등)
14. "select_option" - 옵션 선택 (1번, 2번, 텍스트로, 파일로 등)
15. "search_query" - 내부 작업일지 DB 검색 (우리 시스템에 저장된 작업일지 기록 조회)
    예: "틸리언 작업 보여줘", "2월 4일 나블리 있어?", "3만원짜리 뭐있어?", "1톤하차 몇번 했어?"
    주의: 작업일지/업무 기록 검색이 아닌 외부 정보 검색은 web_search 사용
16. "stats_query" - 통계/분석 요청
    예: "이번달 총 얼마야?", "오늘 몇건 했어?", "가장 많이 일한 업체", "지난주랑 이번주 비교"
17. "specific_edit" - 특정 건 수정 (조건으로 특정 작업을 수정)
    예: "오늘 틸리언 3만원 → 5만원으로", "어제 나블리꺼 수정해줘"
18. "specific_delete" - 특정 건 삭제 (조건으로 특정 작업을 삭제)
    예: "오늘 틸리언 3만원꺼 삭제해줘", "어제 나블리 양품화 삭제"
19. "multi_entry" - 다중 건 입력 (한번에 여러 작업 입력, 쉼표/그리고로 구분)
    예: "틸리언 하차 3만, 나블리 양품화 2만", "A업체 검수 1만 그리고 B업체 하차 5만"
20. "dashboard" - 대시보드/웹페이지 링크 요청
    예: "대시보드", "웹페이지", "링크 줘", "사이트 주소"
21. "compare_periods" - 기간 비교 요청
    예: "지난주랑 이번주 비교해줘", "1월이랑 2월 비교", "어제랑 오늘 비교"
22. "undo" - 실행취소 (여러 단계 되돌리기)
    예: "되돌려줘", "2개 전 것도 취소", "히스토리 보여줘", "최근 변경 취소"
23. "add_memo" - 작업에 메모 추가
    예: "방금꺼에 긴급 메모 추가", "틸리언꺼 비고에 특이사항 적어줘"
24. "bulk_edit" - 일괄 수정 (여러 건 한번에 수정)
    예: "오늘 틸리언 전부 5만원으로", "이번주 나블리 단가 일괄 수정"
25. "copy_entry" - 복사 기능
    예: "어제꺼 오늘로 복사", "지난주 틸리언꺼 복사해줘"
26. "web_search" - 웹 검색/인터넷 조사 요청 (외부 정보 검색, 회사 정보, 뉴스 등)
    예: "스프링 풀필먼트 회사 조사해줘", "풀필먼트 서비스 검색해줘", "A업체 인터넷에서 찾아봐", "뉴스 검색해줘"
    예: "이 업체 정보 알아봐줘", "~에 대해 조사해줘", "~가 뭐야?", "시장 동향 검색"
    주의: 작업일지 기록 검색이 아닌 외부 정보/회사 정보/일반 지식 검색은 모두 web_search
27. "chat" - 일반 대화/질문 (위 어느 것에도 해당 안됨)
...
## web_search vs search_query 구분 (매우 중요!)
- web_search (외부 웹 검색):
  - "~에 대한 정보", "~에 대해 알려줘", "~가 뭐야?"
  - 사람 이름 + 정보 (예: "홍길동 대표 정보")
  - 회사 정보, 서비스 정보, 뉴스, 시장 동향
  - "조사해줘", "찾아봐", "알아봐줘", "검색해줘"
  - 작업일지와 관련 없는 일반 정보 요청
  
- search_query (내부 작업일지 DB 검색):
  - 반드시 "작업", "일지", "내역", "기록" 등의 키워드 포함
  - "틸리언 작업 보여줘", "어제 나블리 뭐 했어?"
  - 금액, 수량, 날짜 기준 작업일지 조회

## 핵심 규칙
- "~에 대한 정보", "~대표", "~회사" → 무조건 web_search
- "작업", "일지", "내역" 없이 정보만 요청 → web_search
- 애매하면 chat
