...
def mode_feature_guide() -> str:
    return (
        ...
        "\n"
        "• 일지모드\n"
        "  작업일지 등록·조회·집계·직전/특정 기록 수정, 작업 단가 이력.\n"
        "  수선일지는 조회모드 또는 수선모드에서 확인합니다.\n"
        "  예: 틸리언 하차 3만원 / 오늘 작업일지 보여줘 / 방금 거 수량 5건으로\n"
        "  시작: 일지모드 시작\n"
        "\n"
        "• 수선모드\n"
        "  수선일지 등록·사진 첨부·조회·집계·직전/특정 기록 수정, 수선 가격.\n"
        "  작업일지는 조회모드 또는 일지모드에서 확인합니다.\n"
        "  예: 구멍 바느질 1500원 / 오늘 수선일지 보여줘 / 두 번째 거 금액 2천원으로\n"
        "  시작: 수선모드 시작\n"
        "\n"
        "• 조회모드\n"
        "  작업일지 검색·목록·건수·수량·금액\n"
