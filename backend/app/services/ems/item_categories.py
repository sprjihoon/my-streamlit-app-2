"""EMS 인보이스 품목 카탈로그 (HS코드 6자리). Infront item-categories 이식."""
...
def suggest_item_categories(query: str = "", limit: int = 20) -> list[dict[str, Any]]:
    q = (query or "").strip().lower().replace(" ", "")
    if not q:
        return ITEM_CATEGORIES[: max(1, min(limit, len(ITEM_CATEGORIES)))]
    hits: list[dict[str, Any]] = []
    for cat in ITEM_CATEGORIES:
        hay = f"{cat['name_ko']}{cat['name_en']}{cat['hs_code']}".lower().replace(" ", "")
        hs = cat["hs_code"]
        if q in hay or hs.startswith(q) or (q.isdigit() and hs.startswith(q)):
            hits.append(cat)
