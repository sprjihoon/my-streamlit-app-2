...
QUERY_HINT_REPAIR = "조회모드에서 확인할 수 있어요. 현재 수선 입력은 그대로 유지할까요?"
QUERY_HINT_JOURNAL = "조회모드에서 확인할 수 있어요. 현재 일지 입력은 그대로 유지할까요?"
...
def looks_like_query_read(text: str) -> bool:
    ...
    if looks_like_last_saved_update(text):
        return False
    if looks_like_last_saved_show(text) or looks_like_repair_logs(text):
        return True
    return any(
        c in compact
        for c in (
...
def should_skip_readonly(text: str, nlu=None) -> bool:
    ...
    if action in {ACTION_LIST, ACTION_LATEST, ACTION_COUNT, ACTION_STATS, ACTION_GROUP}:
        return True
...
def infer_query_fallback(text: str, context: Optional[Dict[str, Any]] = None) -> Any:
    ...
    compact = _compact(text)
    entity = prev.get("entity") or "repair_log"
...
def _merge_filters(nlu, text: str, prev: Optional[Dict[str, Any]]) -> Tuple[str, str, Dict[str, Any]]:
    prev = prev or {}
    compact = _compact(text)
    entity = getattr(nlu, "entity", None) or prev.get("entity") or "repair_log"
    action = getattr(nlu, "action", None) or prev.get("action") or ACTION_LIST
    if action in {ACTION_SHOW_HELP, ACTION_START_MODE, ACTION_QUERY_CATALOG} and looks_like_query_read(text):
        inferred = infer_query_fallback(text, {"query_context": prev})
...
def render_list(entity: str, result: Dict[str, Any], filters: Dict[str, Any]) -> str:
    ...
    for row in rows[:50]:
        if entity == "repair_log":
            lines.append(
                f"• {row.get('date') or '-'} / {row.get('vendor') or '-'} / "
            ...
            lines.append(f"• {date or '-'} / {vendor or '-'} / {work or '-'} / {qty}건 / {int(total):,}원")
    return "\n".join(lines)
...
def execute_query(
    ...
    entity, action, filters = _merge_filters(nlu, text, prev)
    ...
    if action == ACTION_LATEST or looks_like_last_saved_show(text):
        reply = format_last_saved(user_id, channel_id, entity if entity in {"work_log", "repair_log"} else "repair_log")
    ...
    if entity == "repair_price" or (action == ACTION_LOOKUP_PRICE and entity != "work_price"):
        ...
        result = _safe_tool(
            "lookup_repair_price",
            {"work_type": work or text, "vendor": filters.get("vendor"), "product": filters.get("product")},
            user_id,
        )
