...
// ─────────────────────────────────────────────
// 타입 (inbound-overview/page.tsx에서 복사)
// ─────────────────────────────────────────────
...
function chip(color: string, bg: string, text: string, small = false) {
  return (
...
// 불량·수선 서브모달
// ─────────────────────────────────────────────
function SubLogModal({ type, allItems, onClose }: { type: 'defect' | 'repair'; allItems: ItemDetail[]; onClose: () => void }) {
  const [lightbox, setLightbox] = useState<string | null>(null);
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 'min(96vw, 780px)', maxHeight: '88vh', background: C.card, borderRadius: 14, boxShadow: '0 28px 70px rgba(0,0,0,0.3)', zIndex: 2001, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {data && (
          <>
            ...
            {data.batches.map(batch => (
              <div key={batch.id} style={{ background: C.card, borderRadius: 10, border: `1px solid ${C.border}`, marginBottom: '1rem', overflow: 'hidden' }}>
                {/* 도매처 헤더 */}
                <div style={{ padding: '0.75rem 1rem', background: '#f8fafc', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', borderBottom: `1px solid ${C.border}` }}>
                  <span style={{ fontWeight: 700, fontSize: '0.95rem', color: C.text }}>{batch.wholesale}</span>
                  {batch.status === 'closed'
                    ? <span style={{ background: C.successLight, color: C.success, borderRadius: 4, padding: '1px 8px', fontSize: '0.75rem', fontWeight: 600 }}>마감</span>
                    : <span style={{ background: C.warnLight, color: C.warn, borderRadius: 4, padding: '1px 8px', fontSize: '0.75rem', fontWeight: 600 }}>진행중</span>
                  }
                  ...
                  {batch.janggi_url && (
                    <span onClick={() => setLightbox(batch.janggi_url!)} style={{ display: 'flex', alignItems: 'center', gap: 4, cursor: 'zoom-in', background: '#e0f2fe', borderRadius: 5, padding: '2px 8px', fontSize: '0.75rem', color: '#0284c7', fontWeight: 600 }}>
                      📄 장끼 사진
...
export default function InboundOverviewDetailPage() {
  return (
    <Suspense fallback={<div style={{ padding: '2rem', textAlign: 'center', color: '#64748b' }}>로딩 중...</div>}>
      <DetailPageInner />
    </Suspense>
  );
}
