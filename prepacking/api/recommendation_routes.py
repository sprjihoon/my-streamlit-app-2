    ...
    for sup in suppliers:
        ...
        for p in preds:
            ...
            all_items.append({
                ...
                "items": p.get("items", []),
                "predicted_qty": qty,
                "stat_qty": int(p.get("stat_qty", 0)),
                "ml_qty": int(p.get("ml_qty", 0)),
                "model_used": p.get("model_used", "statistical"),
                "ml_accuracy": round(float(p.get("ml_accuracy", 0)), 3),
                "gpt_reason": p.get("gpt_reason", ""),
